...
export function createCanvasMcpServer(forward: Forward): ReturnType<typeof createSdkMcpServer> {
  const tools = [
    ...
    tool(
      'create_code_node',
      'Create a code node: a frame whose children are generated by running JSX source. Use it for anything repetitive, data-driven or interactive instead of building nodes one by one. The source must default-export a component of the canvas primitives (Frame, Rectangle, Ellipse, Text) with web-style props; useState/useEffect/useMemo/useRef are ambient and import statements are not available. If the result carries an error, read it, fix the source with set_code_source and check again.',
      {
        parentId: shapeArgs.parentId,
        x: z.number(),
        y: z.number(),
    ...
    tool(
      'set_code_source',
      'Rewrite a code node’s source, its props, or both, and re-run it. Returns an error string when the new source fails to compile or run; the previous output stays on the canvas until a run succeeds.',
      {
        nodeId,
